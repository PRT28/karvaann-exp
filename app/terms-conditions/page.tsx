    ...
    <main className="font-sans text-[#4b3b2b]">
      ...
      <section className="px-6 md:px-20 py-12 md:py-16">
        <div className="max-w-5xl">
          <div className="inline-flex font-bold items-center gap-3 bg-[#FFC709] text-[#3b2a1f] font-sans uppercase px-4 py-2 rounded-sm text-[16px] md:text-[20px]">
            Terms &amp; Conditions
          </div>
          <p className="mt-6 text-[14px] md:text-[16px] leading-7 font-sans">
        ...
        <div className="mt-10 grid grid-cols-1 gap-10 max-w-5xl">
          {data.terms.map((section) => (
            <div key={section.title} className="border-b border-[#E7D9B0] pb-10">
              <h3 className="font-sans uppercase text-[22px] md:text-[30px] text-[#3f2f23]">
                {section.title}
              </h3>
              <div className="mt-4 space-y-3 text-[14px] md:text-[16px] leading-7 font-sans">
                {section.body.map((p) => (
                  <p key={p}>{p}</p>
                ))}
              </div>
              ...
              {section.bullets && section.bullets.length > 0 ? (
                <ul className="mt-4 list-disc list-inside space-y-2 text-[14px] md:text-[16px] leading-7 font-sans">
